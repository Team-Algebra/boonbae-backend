...
//    Page<Funding> findByApproved(Pageable pageable, boolean check);
    ...
    @Query("SELECT f FROM Funding f WHERE f.isApproved = true ORDER BY f.openDate DESC")
    List<Funding> findByIsApprovedTrueOrderByOpenDateDesc();

    //후원자 수 많은 순
    @Query("SELECT f FROM Funding f WHERE f.isApproved = true ORDER BY (f.currentAmount / f.supportingAmount) DESC")
    List<Funding> findByIsApprovedTrueOrderByCurrentAmountDivSupportingAmount();

    //마감임박 순
    @Query("SELECT f FROM Funding f WHERE f.isApproved = true ORDER BY f.closeDate")
    List<Funding> findByIsAndApprovedTrueOrderByCloseDateAsc();
    ...
    @Query(value = "select f From Funding f where f.title =:title")
